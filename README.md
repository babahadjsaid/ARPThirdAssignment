...
- In server mode, Process A waits for incoming messages and displays them on the screen. The user can quit this mode by entering 'q' and return to the main menu.
- In client mode, Process A prompts the user to enter messages which are then sent to the server. The user can quit this mode by entering 'q' and return to the main menu.<br/> 
...
- `ProcessA.cpp` has been modified to include additional functionality in the form of three different modalities: normal mode, server mode, and client mode. In normal mode, it creates a 2D view where the user can visualize and move the circle. At each instant of time it copies the bit map and sends it to ProcessB through shared memory. If the user clicks the print button it saves the current instance of the bit map in a form of image. In server mode, it waits for messages from clients and in client mode, it allows the user to insert messages. If the user enters 'q' it quits the current mode and goes back to the main menu. If the user clicks 'cntrl+c' it quits the entire project.
